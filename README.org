...
This document supports  the [[./shelf.org][SHELF Standard]].  In  addition to providing
rationale for the  standard, it documents the  function families which
support an implementation of the standard. 

*IT is TAKING over the job  from [[./auxlib.org][AUXLIB]]*.  All families are as before,
except  unique ~auxlib~  functions (no  other family)  are now  in the
*util* group  (a collection  without a  family).  This  paragraph will
...
embedded  in  this  document  as  [[./tangle.org][tangled  fragments]].   The  ancillary
...
Their code is shown below, collected by family; The user guides for
the function families are now separate documents.

** om -- the object-model, or object-method for naming functions
** report -- or *assert* for verifying function arguments

A reporting function may be used in any shell function which needs
to assert its arguments as to number, type of file, or string length,
and any assertion you can support with a function.
...
Notice the usage above.  It's a positive statement to assert an
argument isn't something, and return a failure when that's the case.
So, the user function lines up as many assertions on it's arguments as
needed.  The first one to fail produces the required message and
returns from the function with a message on /stderr/ from
*report_usage*.  Otherwise the function executes as desired.
...
The *om_iam* function is the main user function.  It must be called by
the family's /_init/ function.  When called, it creates these
following functions, where subfunctions are appended to the family
name. e.g. /family_list/:
1. /family/ -- uses *om_generic* to supply help and firsttime
   functionality
1. /_firsttime/ -- executed the first time the family is loaded,
   whose default action is to supply the copyright notice for 
   the family.
1. /_list/ -- if not already defined, produces the list
   of family member functions
...
#+BEGIN_SRC sh :tangle ./inc/om.0 :comments both :padline no
copyright_om () 
{ 
...
is a /family/, the only necessary call is to *om_iam*
...
** trace family
#+BEGIN_SRC sh :tangle ./inc/trace.0
function trace_base
{ 
    printf "TRACE %s ( %s )\n" ${FUNCNAME[1]} "$*" 1>&2
}
...
#+BEGIN_SRC sh :tangle ./src/shelf.install :padline no
...
#+BEGIN_SRC txt :tangle ./inc/shelf.all :padline no
@include util.0
@include report.0
@include shdoc.0
@include trace.0
...
#+BEGIN_SRC txt :tangle ./inc/shelf.0 :padline no
shelf_init ()
{
     om_init
     om_iam
...
verify   the  ~shelflib~   conformance  to   this  standard.    A  bit
